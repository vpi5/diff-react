(() => {
  ...
  const setAttribute = (dom, key, value) => {
    if (typeof value === 'function' && key.startsWith('on')) {
      ...
      dom.__likeReactHandlers = dom.__likeReactHandlers || {};
      dom.removeEventListener(eventType, dom.__likeReactHandlers[eventType]);
      dom.__likeReactHandlers[eventType] = value;
      dom.addEventListener(eventType, dom.__likeReactHandlers[eventType]);
  ...
  const _patch = (dom, vdom, parent = dom.parentNode) => {
    ...
    if (typeof vdom === 'object' && typeof vdom.type === 'function') {
      ...
      [].concat(...dom.childNodes).map((child, index) => {
        const key = child.__likeReactKey || `__index_${index}`;
        pool[key] = child;
      });
      ...
      for (const key in pool) {
        if (Object.prototype.hasOwnProperty.call(pool, key)) {
          const instance = pool[key].__likeReactInstance;
